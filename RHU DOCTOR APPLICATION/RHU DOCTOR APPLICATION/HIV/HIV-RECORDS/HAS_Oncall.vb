...
Imports System.IO
Public Class HAS_Oncall
    Private Sub HAS_Oncall_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DisplayPatientsInQueue()
        Try
            ...
            Dim doctorID As String = HAS_CLINIC.DoctorIDLBL.Text
    ...
    Private Sub SaveDoctorDiagnosis_Click(sender As Object, e As EventArgs) Handles SaveDoctorDiagnosis.Click
        Try
            ...
            If String.IsNullOrWhiteSpace(txtHIVStatus.Text) OrElse
           String.IsNullOrWhiteSpace(txtSTIStatus.Text) OrElse
           String.IsNullOrWhiteSpace(txtHIVMedication.Text) OrElse
           String.IsNullOrWhiteSpace(txtSTIMedication.Text) OrElse
           String.IsNullOrWhiteSpace(txthealthStatus.Text) Then
            ...
            If confirmationResult = DialogResult.Yes Then
                ' User confirmed to save the treatment information
                Dim doctorName As String = HAS_CLINIC.DoctorNameLBL.Text
                Dim patientID As String = lblPatientID.Text
                Dim hivAIDSStatus As String = txtHIVStatus.Text
                Dim stiStatus As String = txtSTIStatus.Text
                Dim hivAIDSMedication As String = txtHIVMedication.Text
                Dim stiMedication As String = txtSTIMedication.Text
                Dim healthStatus As String = txthealthStatus.Text

                Dim result As Boolean = AddTreatmentHAS(doctorName, patientID, hivAIDSStatus, stiStatus, hivAIDSMedication, stiMedication, healthStatus)

                If result Then
                    MessageBox.Show("Treatment information saved successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    ...
                    ClearLabResultDetails()
                Else
                    MessageBox.Show("Failed to save treatment information.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            Else
                ' User chose not to save, provide information or take any action accordingly
            End If
        Catch ex As Exception
    ...
    Private Sub ClearCheckbox()
        For Each i As Integer In txtHIVStatus.CheckedIndices
            txtHIVStatus.SetItemCheckState(i, CheckState.Unchecked)
        Next

        For Each i As Integer In txtSTIStatus.CheckedIndices
            txtSTIStatus.SetItemCheckState(i, CheckState.Unchecked)
        Next

        For Each i As Integer In txtHIVMedication.CheckedIndices
            txtHIVMedication.SetItemCheckState(i, CheckState.Unchecked)
        Next

        For Each i As Integer In txtSTIMedication.CheckedIndices
            txtSTIMedication.SetItemCheckState(i, CheckState.Unchecked)
        Next
    ...
    Private Sub CallPatientVTN_Click(sender As Object, e As EventArgs) Handles CallPatientVTN.Click
        If PatientQueueList.SelectedItem IsNot Nothing Then
            ...
            Dim postText As String = "To the clinic.  Doctor" & HAS_CLINIC.DoctorNameLBL.Text
