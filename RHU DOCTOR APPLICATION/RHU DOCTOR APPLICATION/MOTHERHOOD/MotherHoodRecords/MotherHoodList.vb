...
Public Class MotherHoodList
    Private Sub MotherHoodList_Load(sender As Object, e As EventArgs) Handles MyBase.Load

    End Sub

    Private Sub LoadTreatmentData()
        Try
            If txtSearchPregnancyTest.SelectedIndex <> -1 Then
                Dim selectedTreatment As String = txtSearchPregnancyTest.SelectedItem.ToString()

                Dim selectQuery As String = "SELECT DoctorName, CONCAT(P.FirstName, ' ', P.LastName) AS PatientName, " &
                                    "PregnancyTest, DateRecorded " &
                                    "FROM treatment_motherhood T " &
                                    "INNER JOIN patients P ON T.PatientID = P.PatientID " &
                                    "WHERE PregnancyTest = @SearchPregnancyTest"

                ' Assuming txtSearchPregnancyTest is a TextBox where you set the search value
                Dim searchValue As String = txtSearchPregnancyTest.Text

                Dim parameters As New List(Of MySqlParameter) From {
                New MySqlParameter("@SearchPregnancyTest", searchValue)
            }
    ...
    Private Sub LoadTreatmentDataByDateRange(startDate As DateTime, endDate As DateTime)
        Try
            ' Validate if a value is selected in the ComboBox
            If txtSearchPregnancyTest.SelectedIndex <> -1 Then
                Dim selectedTreatment As String = txtSearchPregnancyTest.SelectedItem.ToString()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles SearchPatientTest.Click
        LoadTreatmentData()
        DisplayRowCount()
    End Sub

    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles SearchWithDate.Click
