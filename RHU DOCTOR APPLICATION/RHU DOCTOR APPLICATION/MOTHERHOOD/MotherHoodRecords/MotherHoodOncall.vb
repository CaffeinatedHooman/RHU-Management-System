...
Imports System.IO
Public Class MotherHoodOncall
    Private Sub MotherHoodOncall_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DisplayPatientsInQueue()
        Try
            ...
            Dim doctorID As String = MotherHoodClinic.DoctorIDLBL.Text
    ...
    Private Sub SaveDoctorDiagnosis_Click(sender As Object, e As EventArgs) Handles SaveDoctorDiagnosis.Click
        Try
            ...
            If String.IsNullOrWhiteSpace(txtPregnancyTest.Text) OrElse
           String.IsNullOrWhiteSpace(txtUltrasoundResult.Text) OrElse
           String.IsNullOrWhiteSpace(txtPatientComplains.Text) OrElse
           String.IsNullOrWhiteSpace(txtPatientHistory.Text) OrElse
           String.IsNullOrWhiteSpace(txtPelvicResult.Text) OrElse
           String.IsNullOrEmpty(txtMedications.Text) Then

                MessageBox.Show("Please fill in all required fields.", "Incomplete Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return ' Exit the method without proceeding
            End If

            ' All required fields are filled, proceed with saving the treatment information
            Dim confirmationResult As DialogResult = MessageBox.Show("Do you want to save the treatment information?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question)

            If confirmationResult = DialogResult.Yes Then
                Dim doctorName As String = MotherHoodClinic.DoctorNameLBL.Text
                Dim patientID As String = lblPatientID.Text
                Dim pregnancyTest As String = txtPregnancyTest.Text
                Dim ultrasoundResult As String = txtUltrasoundResult.Text
                Dim patientComplains As String = txtPatientComplains.Text
                Dim patientHistory As String = txtPatientHistory.Text
                Dim pelvicResult As String = txtPelvicResult.Text
                Dim medications As String = txtMedications.Text

                Dim result As String = AddTreatmentMotherhood(doctorName, patientID, pregnancyTest, ultrasoundResult, patientComplains, patientHistory, pelvicResult, medications)
    ...
    Private Sub PopulateTreatmentDateListBox()
        Dim patientID As String = lblPatientID.Text
        Dim distinctDates As List(Of DateTime) = GetDistinctTreatmentDateMotherHood(patientID)
    ...
    Private Sub DisplayTreatmentDetails(selectedDate As DateTime)
        Try
            Dim patientID As String = lblPatientID.Text
            Dim treatmentTable As DataTable = GetTreatmentMotherhoodRecordsByDate(patientID, selectedDate)
            ...
            If treatmentTable IsNot Nothing AndAlso treatmentTable.Rows.Count > 0 Then
                ...
                Dim pregnancyTest As String = Convert.ToString(treatmentRow("PregnancyTest"))
                Dim ultrasoundResult As String = Convert.ToString(treatmentRow("UltrasoundResult"))
                ...
                Dim pelvicResult As String = Convert.ToString(treatmentRow("PelvicResult"))
                ...
                txtPregnancyTestPast.Text = pregnancyTest
                txtUltrasoundResultPast.Text = ultrasoundResult
                txtPatientComplainsPast.Text = patientComplains
                txtPatientHistoryPast.Text = patientHistory
                txtPelvicResultPast.Text = pelvicResult
                txtMedicationsPast.Text = medications
                txtDateRecordedPast.Text = dateRecorded.ToString("yyyy-MM-dd HH:mm:ss")
            Else
                ' MessageBox.Show("No treatment records found for the specified date.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                ClearTreatmentDetails()
            End If
        Catch ex As Exception
            MessageBox.Show($"An error occurred while displaying treatment details: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub ClearTreatmentDetails()
        ...
        For Each itemIndex As Integer In txtPregnancyTest.CheckedIndices
            txtPregnancyTest.SetItemChecked(itemIndex, False)
        Next
        txtUltrasoundResult.Text = ""
        txtPatientComplains.Text = ""
        txtPatientHistory.Text = ""
        txtPelvicResult.Text = ""
        txtMedications.Text = ""

        txtDoctorNamePast.Text = "="
        txtPregnancyTestPast.Text = "="
        txtUltrasoundResultPast.Text = "="
        txtPatientComplainsPast.Text = "="
        txtPatientHistoryPast.Text = "="
        txtPelvicResultPast.Text = "="
    ...
    Private Sub CallPatientVTN_Click(sender As Object, e As EventArgs) Handles CallPatientVTN.Click
        If PatientQueueList.SelectedItem IsNot Nothing Then
            ...
            Dim postText As String = "To the clinic.  Doctor" & MotherHoodClinic.DoctorNameLBL.Text
